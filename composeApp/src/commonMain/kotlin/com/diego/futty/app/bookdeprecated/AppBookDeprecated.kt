...
@Composable
@Preview
fun AppBookDeprecated() {
    MaterialTheme {
        val navController = rememberNavController()
        NavHost(
            navController = navController,
            startDestination = RouteBookDeprecated.BookGraph
        ) {
            navigation<RouteBookDeprecated.BookGraph>(
                startDestination = RouteBookDeprecated.BookList
            ) {
                composable<RouteBookDeprecated.BookList>(
                    ...
                    BookListScreenRoot(
                        ...
                        onBookClick = { book ->
                            ...
                            navController.navigate(
                                RouteBookDeprecated.BookDetail(book.id)
                            )
                        },
                        onBack = {}
                    )
                }
                composable<RouteBookDeprecated.BookDetail>(
...
private inline fun <reified T: ViewModel> NavBackStackEntry.sharedKoinViewModel(
    ...
    val navGraphRouteBookDeprecated = destination.parent?.route ?: return koinViewModel<T>()
    val parentEntry = remember(this) {
        navController.getBackStackEntry(navGraphRouteBookDeprecated)
    }
